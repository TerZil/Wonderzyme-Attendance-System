...
if page == "👤 Add Employee Profile":
    ...
    if employees_df.empty:
        ...
        if submitted:
            ...
            if total_hrs is None:
                ...
                # ─── SUCCESS MESSAGE ───
                ot_text = f" (incl. {ot_hrs:.1f} hrs OT)" if ot_hrs > 0 else ""
                
                st.success(f"✅ **Attendance Recorded Successfully for {name}!**")
                
                # Optional: Show a nice summary card of what was just recorded
                st.info(f"""
                **Summary for {today_str}:**
                * **Time:** {ti_s} - {to_s}
                * **Total Hours:** {total_hrs:.1f} hrs{ot_text}
                * **Task:** {workload if workload else 'No tasks specified'}
                """)
                
                # Note: I removed st.rerun() here so the message stays visible for the user to see.
                # If you want the page to refresh immediately, keep st.rerun() but the message will disappear.

elif page == "📄 Records":
    # ─── HEADER WITH DATE ───
    st.subheader("📋 Attendance Management")
    st.markdown(f"📅 **Date:** {today_str}") # Restored the readable date here
    ...
    with col_a:
        st.metric("Total Personnel", len(today_df))
    with col_b:
        total_hrs_sum = today_df['total_hours'].sum() if not today_df.empty else 0
        st.metric("Total Man-Hours", f"{total_hrs_sum:.1f}")
    with col_c:
        ot_hrs_sum = today_df['overtime_hours'].sum() if not today_df.empty else 0
        st.metric("Total Overtime", f"{ot_hrs_sum:.1f}", delta_color="inverse")

    st.markdown("---")

    if today_df.empty:
        st.info(f"No attendance records entered for today ({today_db_format}).")
    else:
        st.write(f"### 📝 Timesheet for {today_db_format}")
        st.caption("Double-click a cell to edit. Select a row and press 'Delete' on your keyboard to remove it.")

        # ─── THE INTERACTIVE TABLE ───
        edited_df = st.data_editor(
            today_df,
            key="main_attendance_editor",
            use_container_width=True,
            hide_index=True,
            num_rows="dynamic",
            column_config={
                "id": None,  
                "name": st.column_config.TextColumn("Employee Name", width="medium", disabled=True),
                "time_in": st.column_config.TextColumn("Clock In (HH:MM)", width="small"),
                "time_out": st.column_config.TextColumn("Clock Out (HH:MM)", width="small"),
                "total_hours": st.column_config.NumberColumn("Total Hrs", format="%.1f", disabled=True),
                "overtime_hours": st.column_config.NumberColumn("OT Hrs", format="%.1f", disabled=True),
                "workload": st.column_config.TextColumn("Task/Workload", width="large"),
        ...
        with btn_col1:
            if st.button("💾 Sync & Save Changes", use_container_width=True, type="primary"):
                conn = sqlite3.connect(DB_FILE)
                try:
                    conn.execute("DELETE FROM attendance WHERE date = ?", (today_db_format,))
                    for _, row in edited_df.iterrows():
                        t_hrs, o_hrs = time_to_hours(row['time_in'], row['time_out'])
                        conn.execute("""
                            INSERT INTO attendance (name, date, time_in, time_out, total_hours, overtime_hours, workload, remarks)
                            VALUES (?,?,?,?,?,?,?,?)
                        """, (row['name'], today_db_format, row['time_in'], row['time_out'], t_hrs, o_hrs, row['workload'], row['remarks']))
                    conn.commit()
                    st.success(f"Changes for {today_db_format} saved successfully!")
                    st.rerun()
                except Exception as e:
        ...
        with btn_col2:
            ...
            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                edited_df.to_excel(writer, index=False, sheet_name="Daily_Attendance")
            output.seek(0)
            
            st.download_button(
                label="📥 Export to Excel",
                data=output,
                file_name=f"Wonderzyme_Attendance_{today_db_format}.xlsx",
